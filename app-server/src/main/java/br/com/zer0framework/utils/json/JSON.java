...
/**
 * Simple JSON parsing for Java. This class provides a rudimentary callback
 * implementation that may be passed to @see json.Lexer . In case you are
 * interested in writing your own specialized callback to use with json.Lexer
 * the callback contained herein may be a good starting point.
 * <p>
 * JSON objects (`{"bla":1}`) are converted to `java.utils.Map's` (Maps in the
 * interface HashMaps for the implementation), JSON arrrays are converted to
 * `java.util.List's` (LinkedList for the implementation), JSON Strings become
 * Java Strings, Numbers become `BigDecimal`, `true` and `false` are boolean and
 * null is, well, null.
 *
 * <h2>Usage</h2> <code>
 *
 ...
 * </code>
 * <p>
 * In the example above, `o` will be a `java.util.Map` containing three keys,
 * "a", "b" and "dingdong", each Strings. "a"'s value is a BigDecimal, "b"'s an
 * array containing the BigDecimal values 1, 2, and 3 and "dingdong"'s value is
 * another Map ...
 * <p>
 * The intended use case for this is to write non-blocking webservices, this
 * interface is meant to provide the functionality to process any scrap of data
 * that happens to be available on the network. This requires a slightly more
 * elaborate interface than the simple verson above:
 *
 * <code>
 * JSON json = new JSON();
 ...
 * <h2>Accepted JSON</h2>
 * <p>
 * This implementation should be able to handle any JSON conforming to JSON as
 * described here (http://json.org).
 * <p>
 * Technically, this parser accepts a superset of JSON that allows redundant ':'
 * and ',' inside of JSON objects and Arrays to be left out, so it would accept:
 *
 * <code>
...
public class JSON {
	...
	/**
	 * Mungle up an Object into JSON. There are a bunch of cases this can't handle,
	 * for example: just any old stuff.
	 * <p>
	...
	/**
	 * Returns whether the parser is in a consistant, balanced state. Once the
	 * parser is `done` passing further data to it via `parse` will trigger an
	 * Exception.
	 */
	public boolean done() {
		return this.cb.done;
	...
	/**
	 * Retrieve the results of the parse. You need to ensure that the complete JSON
	 * object has been passed to parse, else this will throw and Exception. Ideally,
	 * call `done()` before trying to retrieve the results
	 */
	public Object obj() {
		if (!done()) {
